...
function DrawRadar()
	...
	if cameraRatio.curr <= 1 / 8 then
		...
		boxSize = (cameraRatio.curr * 8 - 1) / cameraRatio.curr / 16
...
function DrawMouse1()
	...
	if mode_manager.time() - mouseStart >= 2.0 then
		...
		realPos.x = (mousePos.x - ship.x + shipAdjust) * cameraRatio.curr
		realPos.y = (mousePos.y - ship.y) * cameraRatio.curr
		
		if realPos.x > panels.right.center.x - panels.right.width / 2 - 10 then
			mousePos.x = (panels.right.center.x - panels.right.width / 2 - 10) / cameraRatio.curr - shipAdjust + ship.x
		elseif realPos.x < panels.left.center.x + panels.left.width / 2 then
			mousePos.x = (panels.right.center.x + panels.right.width / 2) / cameraRatio.curr - shipAdjust + ship.x
		end
		
		if realPos.y > WINDOW.height / 2 - 10 then
			mousePos.y = (WINDOW.height / 2 - 10) / cameraRatio.curr + ship.y
		elseif realPos.y < -WINDOW.height / 2 + 10 then
			mousePos.y = (-WINDOW.height / 2 + 10) / cameraRatio.curr + ship.y
		end
		
		graphics.draw_line({ x = - camera.w / 2 + ship.x, y = mousePos.y }, { x = mousePos.x - 20 / cameraRatio.curr, y = mousePos.y }, 1.0, ClutColour(4, 8))
		graphics.draw_line({ x = camera.w / 2 + ship.x, y = mousePos.y }, { x = mousePos.x + 20 / cameraRatio.curr, y = mousePos.y }, 1.0, ClutColour(4, 8))
		graphics.draw_line({ x = mousePos.x, y = -camera.h / 2 + ship.y }, { x = mousePos.x, y = mousePos.y - 20 / cameraRatio.curr }, 1.0, ClutColour(4, 8))
		graphics.draw_line({ x = mousePos.x, y = camera.h / 2 + ship.y }, { x = mousePos.x, y = mousePos.y + 20 / cameraRatio.curr }, 1.0, ClutColour(4, 8))
	end
end
